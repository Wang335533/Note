...
2.4.0 延续暖白、炭黑、浅灰与陶土橙的克制视觉，并让笔记整理和字号调整更直接：每篇笔记都可从列表或右键菜单移入未分类、笔记本根目录或任意一级文件夹；工具栏新增 Word 式“缩小一号 / 放大一号”按钮和快捷键，并沿固定字号阶梯变化。窗口、公式、行距、文件夹与本地升级保留能力不变。
...
- 笔记本优先的两层结构：全部笔记、未分类、普通笔记本、笔记本内一级文件夹与废纸篓；文件夹支持新建、重命名、移动、删除和恢复，不加入标签或多级嵌套
- 笔记列表中的更多按钮和右键菜单都可打开“移动到…”；笔记可在未分类、笔记本根目录及不同一级文件夹之间移动
- 笔记列表中的“笔记 +”入口始终可见；在文件夹内创建时自动保留当前位置，标题立即获得焦点
- 正文采用真正的所见即所得富文本；标题、字体、字号、段落行距、加粗、斜体、下划线、删除线、列表、清单、引用、代码、链接和格式刷都直接作用于文字，不显示 Markdown 或 HTML 标签；工具栏及 `Ctrl + [` / `Ctrl + ]` 可按固定字号阶梯缩小或放大一号
...
- 无边框窗口支持四边与四角自由缩放，最小为 420×660，不再设置人为最大尺寸；支持标准最大化/还原，分别记住最大化状态和普通窗口位置，并在多显示器工作区内校正
