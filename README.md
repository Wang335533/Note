...
- 富文本支持标题、固定字体/字号、行距、四种段落对齐和正文首行缩进，并提供 Word 式格式刷与清除格式。格式刷单击应用一次、双击锁定连续使用，按 `Esc` 或再次单击退出；局部选区只复制文字格式，完整段落或光标所在段落同时复制段落格式，链接与表格结构不会被当作格式清除。对齐使用 `Ctrl + L/E/R/J`，字号可用 `Ctrl + [` / `Ctrl + ]` 逐级调整；混合选区使用 Times New Roman 时只改变西文，中文原有字体保持不变。
