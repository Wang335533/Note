...
1.4.0 版保留现有 Claude 风格的暖白、炭黑、浅灰与陶土橙，并将桌面端和浏览器预览统一到同一个 Store。任务时间段、完成划线动效和手动顺序均保持不变，界面和运行时依赖没有增加常驻负担。
...
```text
release/Note-fast-1.4.0/Note.exe
```

也可以运行 `release/Note-1.4.0-setup.exe` 安装到 Windows。`release/Note-1.4.0-portable.exe` 是免安装备用版，但每次启动都需要解压，速度较慢。
