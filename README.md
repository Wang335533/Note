...
- 富文本支持标题、固定字体/字号、行距、四种段落对齐和正文首行缩进，并提供 Word 式格式刷与清除格式。格式刷单击应用一次、双击锁定连续使用，按 `Esc` 或再次单击退出；局部选区只复制文字格式，完整段落或光标所在段落同时复制段落格式，链接与表格结构不会被当作格式清除。对齐使用 `Ctrl + L/E/R/J`，字号可用 `Ctrl + [` / `Ctrl + ]` 逐级调整；混合字体或字号会明确显示为“多种”，重新选择即可统一。Times New Roman 会同时规范选区和后续输入中的西文、数字，中文及其原有东亚字体保持不变。
...
- 支持一次导入多个 Markdown 文件、导出当前单篇笔记及导出整库。单篇笔记使用系统“另存为”选择目录和文件名，整库使用系统目录选择器；桌面嵌入模式下选择窗口也会可靠显示在前台。导出前会等待标题与正文保存完成，保存或 IPC 异常会显示明确反馈而不会静默失效。单篇或整库导出遇到受管图片时，会写入同名 `.assets` 文件夹并使用普通相对路径；永久删除前不会清除正文或图片。
