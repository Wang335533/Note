...
1.4.1 版保留现有 Claude 风格的暖白、炭黑、浅灰与陶土橙，并将启动工作收敛到首屏所需内容。任务时间段、完成划线动效和手动顺序均保持不变；托盘与全局快捷键会在卡片出现后初始化，发布包不再提供每次启动都要解压的便携版。
...
```text
release/Note-fast-1.4.1/Note.exe
```

也可以运行 `release/Note-1.4.1-setup.exe` 安装到 Windows。为了避免误开后等待解压，1.4.1 起不再生成便携版。
