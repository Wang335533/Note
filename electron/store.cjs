...
function normalizeState(raw, now = new Date()) {
  ...
  settings.dayBoundaryHour = [0, 2, 4, 6].includes(Number(settings.dayBoundaryHour))
    ? Number(settings.dayBoundaryHour)
    : 4;
  settings.windowMode = ["desktop", "normal", "floating"].includes(settings.windowMode)
    ? settings.windowMode
    : "desktop";
  const days = {};
...
function ensureCurrentDay(state, now = new Date()) {
  ...
  const current = localDayKey(now, next.settings.dayBoundaryHour);
...
function applyOperation(state, operation, now = new Date()) {
  ...
  switch (operation.type) {
    ...
    case "settings:set": {
      ...
      if (key === "windowMode") {
        next.settings[key] = ["desktop", "normal", "floating"].includes(operation.value)
          ? operation.value
          : "desktop";
        next.settings.windowModeVersion = 1;
      }
      else if (key === "dayBoundaryHour") next.settings[key] = [0, 2, 4, 6].includes(Number(operation.value)) ? Number(operation.value) : 4;
      else if (key === "windowBounds") next.settings[key] = operation.value && typeof operation.value === "object" ? operation.value : null;
      else next.settings[key] = Boolean(operation.value);
      break;
    }
    default:
